from .common import Info    _VALID_URL = r'https?://(?:www        slug, episode_info = self._get_json(url)
        urls = traverse_obj(
        ...
        return {
            ...
            'thumbnail': traverse_obj(episode_info, 'thumbnail', 'image', expected_type=url_or_none),/(?P<sites_type>episode|videos)/(?P<id>[\w-]+)'
    _TESTS = [{
